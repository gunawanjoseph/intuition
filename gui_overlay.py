...
Fixed for macOS to keep icon always visible.
"""
...
class ContextIcon(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        # macOS-specific window flags for better "always on top" behavior
        ...
        # Timer to periodically raise the window (macOS fix)
        self._raise_timer = QTimer(self)
        self._raise_timer.timeout.connect(self._ensure_on_top)
        self._raise_timer.start(1000)  # Check every second
    
    def _ensure_on_top(self):
        """Ensure the icon stays on top (macOS fix)."""
        if self.isVisible():
            self.raise_()
            self.activateWindow()
    ...
    def showEvent(self, event):
        """Handle show event - ensure position and raise."""
...
class InfoPanel(QWidget):
    ...
    def eventFilter(self, obj, event):
        """Filter events to detect clicks outside the panel."""
...
class OverlayManager:
    ...
    def __init__(self):
        ...
        # Visibility check timer (macOS fix)
        self._visibility_timer: Optional[QTimer] = None
    ...
    def initialize(self):
        ...
        # Setup visibility check timer (macOS fix)
        self._visibility_timer = QTimer()
        self._visibility_timer.timeout.connect(self._check_icon_visibility)
        self._visibility_timer.start(500)  # Check every 500ms
        
        print("[OverlayManager] Initialized")
    
    def _check_icon_visibility(self):
        """Ensure icon stays visible and in correct position (macOS fix)."""
        if self.icon and self.icon.isVisible():
            # Re-position and raise
            self.icon.position_bottom_left()
            self.icon.raise_()
            self.icon.show()
