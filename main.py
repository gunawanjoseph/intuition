...
def main():
    ...
    try:
        ...
        # Create a timer to check for shutdown signal
        # This allows Ctrl+C to work with PyQt6's event loop
        def check_shutdown():
            global _shutdown_requested
            if _shutdown_requested:
                app.stop()
        
        # Get Qt app instance and set up timer
        qt_app = QApplication.instance()
        if qt_app:
            signal_timer = QTimer()
            signal_timer.timeout.connect(check_shutdown)
            signal_timer.start(100)  # Check every 100ms
            app._signal_timer = signal_timer
        
        # Run Qt event loop
        return app.overlay_manager.run()
